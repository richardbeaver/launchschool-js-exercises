...
// When playing around with the above program, our two test rolls result in
// two random values that produce the sample output below (because each dice
// roll produces a random value, your output may differ). The outcome of
// rolling 16 looks correct, but the output when we rolled values 1 and 20
// doesn't make sense. For each roll, only one outcome should be displayed.
...
In the `targetRoll` function, we use a switch statement but do not include
`break` statements at the end of each case clause. This allows execution to
fall through to the following case clauses until all clauses have been
executed.
